            ...
            if ($request->isMethod('post')) {
               ...
               // return Response::json($_FILES);
                ...
                if (File::size($file_path) < 26214400) {
                    ...
                    Files::create(['group_id' => $this->group->id, 'user_id' => Auth::user()->id, 'filename' => $origin_name, 'path' => $this->group->id.'/'.$origin_name]);
                    ...
                    $answer = ['success'=>false, 'message'=>'Розмір файлу занадто великий, максимальний розмір 25мб'];
                }
                if ($request->ajax()) {
                ...
                try {
                    ...
                    foreach ($users as $k=>$v) {
                        if ($input['type'] == 'sms') {
                            ...
                            Mail::send(['text' => 'emails.plain'], ['hello' => $input['text']], function ($message) use ($v) {
            ...
            if ($request->isMethod('post')) {
                ...
                if (!empty($input)) {
                    foreach ($input as $k=>$v) {
                        UserGroups::insert(['user_id'=>$v, 'group_id'=>$this->group->id]);
                    }
        ...
        if ($this->group->owner == Auth::user()->id) {
            ...
            if ($request->isMethod('post')) {
                ...
                try {
                    ...
                    DB::table('user_roles')->insert(['user_id'=>$user->id, 'role_id'=>$input['role']]);
